...

{
  ...
  for (auto & hist : h.at(Constants::DATA_SAMPLE_NAME))
  ...
  // draw_beta(h, "Beta Linear Events");

  // draw_dplane(h, "dplane xz");

  // draw_dplane(h, "dplane yz");

  // draw_n_hits(h, "nhits");

  // draw_n_hits(h, "nhits xz");

  // draw_n_hits(h, "nhits yz");

  // draw_n_tracks(h, "Number of Reco Tracks");

  // draw_n_tracks(h, "First Preselected Reco Track");

  // draw_projected_length(h, "Track #Deltax");

  // draw_projected_length(h, "Track #Deltay");

  // draw_projected_length(h, "Track #Deltaz");
  ...
  // draw_theta(h, "#theta_{xz}");

  // draw_theta(h, "#theta_{yz}");
...

{
  ...
  // TH1 names have to be unique, so let's prepend the type name.
...

{
...

  {
    ...
    if (e.beta_value_matches(Constants::MC_BETA_NAME))

    {
...

      {
...

        {
          ...
          if (type == Constants::DATA_SAMPLE_NAME && e.is_slow())
...

{
  ...
  TH1* data = get(h.at(Constants::DATA_SAMPLE_NAME), hist_name);
  ...
  compare.new_canvas(hist_name, true);

  data->Draw();

  mc->Draw("SAMES");

  // resize_stats_box(mc, 0.135, 0.335, 0.48, 0.88);
...

{
  ...
  compare.new_canvas(canvas_name);
...

{
  ...
  // resize_stats_box(data, 0.635, 0.835, 0.55, 0.95);

  // data->Draw();

  mc->Draw("SAMES");

  // resize_stats_box(mc, 0.435, 0.635, 0.55, 0.95);
...

{
  ...
  compare.new_canvas(hist_name.c_str());

  data->Draw();

  mc->Draw("SAMES");

  // resize_stats_box(mc, 0.58, 0.78, 0.535, 0.935);
...

{
  ...
  // y->SetRangeUser(5e-1, 1.1 * std::max(data->GetMaximum(), mc->GetMaximum()));
  ...
  compare.new_canvas(canvas_name, false, true);

  data->Draw("E HIST");

  // resize_stats_box(data, 0.335, 0.535, 0.48, 0.88);
...

{
  ...
  compare.new_canvas("Track Length");
