...
Event_Info::Event_Info(tree_t const& t, std::string input_file_name) :
...

{
  ...
  if (is_mc())
    ...
    extract_beta_value(input_file_name);
...
bool Event_Info::beta_value_matches(std::string beta_value) const
...
void Event_Info::extract_beta_value(std::string name)

{
  ...
  // std::size_t end = name.find("_birksmod");
...

{
  ...
  result *= Constants::GENERATOR_INCORRECT_MASS / Constants::MONOPOLE_MASS;
...
/*
  ...
  This implements the generator-level velocity vector starting from

  the three available quantities.  The only problem is that there is a

  directional ambiguity since we only have the ratios of the momenta.

  It might be the case that the projected area is the same for the monopole

  coming from either of the two ambiguous directions.  However, I prefer

  to properly implement the generator information at the novasoft level.

 */

// TVector3 Event_Info::gen_velocity_vector() const

// {

//   double dxdz = get("gen_dxdz");

//   double dydz = get("gen_dydz");

//   double p = gen_beta() * Constants::MONOPOLE_MASS;

//   double px = p * std::sin(std::atan(dxdz));

//   double py = p * std::sin(std::atan(dydz));

//   double pz = px / dxdz;

//   double vx = Constants::SPEED_OF_LIGHT * px / Constants::MONOPOLE_MASS;

//   double vy = Constants::SPEED_OF_LIGHT * py / Constants::MONOPOLE_MASS;

//   double vz = Constants::SPEED_OF_LIGHT * pz / Constants::MONOPOLE_MASS;
...
//     std::cout << "Generator Vector:" << std::endl;

//     vel.Print();

//     std::cout << "MC Vector:" << std::endl;

//     mc_velocity_vector().Print();
...

{
  ...
  if (mc_fraction() > Constants::MC_FRACTION_CUT)
...

{
  ...
  if (r2_min() < Constants::R2_MIN_CUT)
...

{
  ...
  if (get("mc_monopole_hit_detector") == 1)
...

{
  ...
  if (get("mc_n_hits_x") < 20)

    return false;

  if (get("mc_n_hits_y") < 20)

    return false;

  

  if (get("mc_dplane_x") < 10)

    return false;

  if (get("mc_dplane_y") < 10)

    return false;

  if (get("mc_length") < 1000)
...

{
  ...
  if (get("n_hits_x", n_track) < 20)

    return false;

  

  if (get("n_hits_y", n_track) < 20)

    return false;

  if (get("dplane_x", n_track) < 10)

    return false;

  if (get("dplane_y", n_track) < 10)

    return false;

  if (get("length", n_track) < 1000)
...

{
  ...
  if (beta() > Constants::SLOW_BETA_CUT)
...

{
  ...
  if (gap_max() > Constants::GAP_MAX_CUT)
...

{
  ...
  return std::min(n_tracks(), Constants::MAX_RECO_N_TRACKS);
...

{
  ...
  if (view == "xz")
    ...
    branch_name = prefix + "_dxdz";

  else if (view == "yz")

    branch_name = prefix + "_dydz";

  else

    assert("\n\n\t\tInvalid View Value!" && false);
